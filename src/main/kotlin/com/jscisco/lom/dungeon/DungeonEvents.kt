...
object DungeonEvents {
    ...
    private fun registerMovementEvent() {
        Zircon.eventBus.subscribe<MoveEntityEvent> { (context, entity, position) ->
            logger.debug("%s is trying to move to %s.".format(entity.entityName, position))
    ...
    private fun registerDamageEvent() {
        Zircon.eventBus.subscribe<DamageEvent> { (context, source, target, amount) ->
            // target.hp -= amount
            logger.info("%s dealt %s damage to %s".format(source.entityName, amount, target.entityName))
            Zircon.eventBus.publish(GameLogEvent("%s damage dealt to %s by %s".format(amount, target.entityName, source.entityName)))
            target.health.hpProperty.value -= amount
            logger.info("%s has %s health remaining.".format(target.entityName, target.health.hp))
    ...
    private fun registerOnHitEvent() {
        Zircon.eventBus.subscribe<OnHitEvent> { (context, source, target) ->
            logger.info("%s hit %s!".format(source.entityName, target.entityName))
    ...
    private fun registerInstigateCombatEvent() {
        Zircon.eventBus.subscribe<InstigateCombatEvent> { (context, source, target) ->
            logger.info("%s instigated combat against %s.".format(source.entityName, target.entityName))
            Zircon.eventBus.publish(OnHitEvent(context, source, target))
            Zircon.eventBus.publish(GameLogEvent("%s instigated combat against %s.".format(source.entityName, target.entityName)))
        }
    }

    private fun registerOpenDoorEvent() {
        Zircon.eventBus.subscribe<OpenDoorEvent> { (context, source, door) ->
            logger.info("%s opened the %s".format(source.entityName, door.entityName))
